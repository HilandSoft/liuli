            ...
            if (!this.Page.IsPostBack)
            {
                ...
                Label_0292:
                    ...
                    for (int j = 0; j < num3; j++)
                    {
                        ...
                        {
                            str5 = Convert.ToDateTime(listByTime.Rows[j]["RepayTime"]).Day.ToString() + "/" + Convert.ToDateTime(listByTime.Rows[j]["RepayTime"]).Month.ToString() + "/" + Convert.ToDateTime(listByTime.Rows[j]["RepayTime"]).Year.ToString();
                        }
                        if (Convert.ToDateTime(listByTime.Rows[j]["OperateTime"]).ToShortDateString() != "1/1/2000")
                        {
                            str6 = Convert.ToDateTime(listByTime.Rows[j]["OperateTime"]).Day.ToString() + "/" + Convert.ToDateTime(listByTime.Rows[j]["OperateTime"]).Month.ToString() + "/" + Convert.ToDateTime(listByTime.Rows[j]["OperateTime"]).Year.ToString();
                        }
                        builder.Append("<td width=\"10%\">" + str5 + "</td>");
