        ...
        private void bnCal2_ServerClick(object sender, EventArgs e)
        {
            ...
            if ((this.txIncome2.Value != "") && (this.txLoan.Value != ""))
            {
                ...
                if (table.Rows.Count > 0)
                {
                    ...
                    string paymentCircleType = this.RadioButtonList2.SelectedValue;
                    try
                    {
                        double num7;
                        switch (paymentCircleType)
                        {
                            case "0":
                    ...
                    Label_0236:
                        num7 = (((0.25 * num6) * addMonths) / ((num * addMonths) + 1.0)) + 1.0;
                        if (num3 > num7)
                        {
                            goto Label_063C;
                        }
                        DateTime nextPayDate = new DateTime(Convert.ToInt32(this.txYy1.Value), Convert.ToInt32(this.txMm1.Value), Convert.ToInt32(this.txDd1.Value), 0x17, 0x3b, 0x3b);
                        /*if (nextPayDate <= SafeDateTime.LocalNow)
                        {
                            goto Label_062A;
                        }
                        DateTime nextDate = nextPayDate;
						*/
                        TimeSpan spanBetwonNextPayDateAndNow = (TimeSpan) (nextPayDate - SafeDateTime.LocalNow);
                        if (nextPayDate <= SafeDateTime.LocalNow)
                        {
                        ...
                        DateTime lastPayDate = new DateTime(0x7d6, 1, 1, 0x17, 0x3b, 0x3b);
                        string str = paymentCircleType;
                        if (str != null)
                        {
                            ...
                            {
                                if (spanBetwonNextPayDateAndNow.Days > 7)
                                {
                                ...
                                if (spanBetwonNextPayDateAndNow.Days < 15)
                                {
                                    nextMonth = nextPayDate.AddDays(7.0);
                                    lastPayDate = nextPayDate.AddDays((double) (7 * addMonths));
                                }
                                else
                                {
                                    nextMonth = nextPayDate;
                                    lastPayDate = nextPayDate.AddDays((double) (7 * (addMonths - 1)));
                                }
                            }
                        }
                        goto Label_05A2;
                    Label_03F6:
                        if (spanBetwonNextPayDateAndNow.Days > 14)
                        {
                            base.Response.Write("<script>alert('Please note that the days to your next payday exceeded your pay interval, please adjust and continue.');</script>");
                            return;
                        }
                        ...
                        if (spanBetwonNextPayDateAndNow.Days < 15)
                        {
                            nextMonth = nextPayDate.AddDays(14.0);
                            lastPayDate = nextPayDate.AddDays((double) (14 * addMonths));
                        }
                        else
                        {
                            nextMonth = nextPayDate;
                            lastPayDate = nextPayDate.AddDays((double) (14 * (addMonths - 1)));
                        }
                        goto Label_05A2;
                    Label_047E:
                        if (spanBetwonNextPayDateAndNow.Days > 0x10)
                        {
                            base.Response.Write("<script>alert('Please note that the days to your next payday exceeded your pay interval, please adjust and continue.');</script>");
                            return;
                        }
                        ...
                        if (spanBetwonNextPayDateAndNow.Days < 15)
                        {
                            ...
                            lastPayDate = LongTermSchedule.GetNextBiMonth(nextPayDate, nextMonth, num5);
                        }
                        else
                        {
                            nextMonth = nextPayDate;
                        ...
                        goto Label_05A2;
                    Label_052A:
                        if (spanBetwonNextPayDateAndNow.Days > 0x1f)
                        {
                            base.Response.Write("<script>alert('Please note that the days to your next payday exceeded your pay interval, please adjust and continue.');</script>");
                            return;
                        }
                        ...
                        if (spanBetwonNextPayDateAndNow.Days < 15)
                        {
                            nextMonth = LongTermSchedule.GetNextMonth(nextPayDate, 1);
                            lastPayDate = LongTermSchedule.GetNextMonth(nextPayDate, addMonths);
                        }
                        else
                        {
                            nextMonth = nextPayDate;
                            lastPayDate = LongTermSchedule.GetNextMonth(nextPayDate, addMonths - 1);
                        }
                    Label_05A2:
                        num *= 0.01;
                        ...
                        this.txEnd.Text = lastPayDate.ToString("dd/MM/yyyy");
                        return;
                    Label_063C:
