namespace YingNet.WeiXing.DB.Data
{
    using System;

    public class HuibackupDT
    {
        private string m_City;
        private DateTime m_DBirth;
        private string m_DNumber;
        private string m_Email;
        private string m_Fname;
        private string m_HTel;
        private int m_id;
        private string m_IsEmployed;
        private string m_Issued;
        private string m_Lname;
        private string m_Mname;
        private string m_Mobile;
        private DateTime m_Modtime;
        private string m_Param1;
        private string m_Param2;
        private string m_Param3;
        private string m_Param4;
        private string m_Param5;
        private string m_Password;
        private string m_Postcode;
        private string m_RAddress;
        private string m_SAddress;
        private string m_State;
        private int m_TMonths;
        private int m_TYears;
        private string m_Username;

        public string City
        {
            get
            {
                return this.m_City;
            }
            set
            {
                this.m_City = value;
            }
        }

        public DateTime DBirth
        {
            get
            {
                return this.m_DBirth;
            }
            set
            {
                this.m_DBirth = value;
            }
        }

        public string DNumber
        {
            get
            {
                return this.m_DNumber;
            }
            set
            {
                this.m_DNumber = value;
            }
        }

        public string Email
        {
            get
            {
                return this.m_Email;
            }
            set
            {
                this.m_Email = value;
            }
        }

        public string Fname
        {
            get
            {
                return this.m_Fname;
            }
            set
            {
                this.m_Fname = value;
            }
        }

        public string HTel
        {
            get
            {
                return this.m_HTel;
            }
            set
            {
                this.m_HTel = value;
            }
        }

        public int id
        {
            get
            {
                return this.m_id;
            }
            set
            {
                this.m_id = value;
            }
        }

        public string IsEmployed
        {
            get
            {
                return this.m_IsEmployed;
            }
            set
            {
                this.m_IsEmployed = value;
            }
        }

        public string Issued
        {
            get
            {
                return this.m_Issued;
            }
            set
            {
                this.m_Issued = value;
            }
        }

        public string Lname
        {
            get
            {
                return this.m_Lname;
            }
            set
            {
                this.m_Lname = value;
            }
        }

        public string Mname
        {
            get
            {
                return this.m_Mname;
            }
            set
            {
                this.m_Mname = value;
            }
        }

        public string Mobile
        {
            get
            {
                return this.m_Mobile;
            }
            set
            {
                this.m_Mobile = value;
            }
        }

        public DateTime Modtime
        {
            get
            {
                return this.m_Modtime;
            }
            set
            {
                this.m_Modtime = value;
            }
        }

        public string Param1
        {
            get
            {
                return this.m_Param1;
            }
            set
            {
                this.m_Param1 = value;
            }
        }

        public string Param2
        {
            get
            {
                return this.m_Param2;
            }
            set
            {
                this.m_Param2 = value;
            }
        }

        public string Param3
        {
            get
            {
                return this.m_Param3;
            }
            set
            {
                this.m_Param3 = value;
            }
        }

        public string Param4
        {
            get
            {
                return this.m_Param4;
            }
            set
            {
                this.m_Param4 = value;
            }
        }

        public string Param5
        {
            get
            {
                return this.m_Param5;
            }
            set
            {
                this.m_Param5 = value;
            }
        }

        public string Password
        {
            get
            {
                return this.m_Password;
            }
            set
            {
                this.m_Password = value;
            }
        }

        public string Postcode
        {
            get
            {
                return this.m_Postcode;
            }
            set
            {
                this.m_Postcode = value;
            }
        }

        public string RAddress
        {
            get
            {
                return this.m_RAddress;
            }
            set
            {
                this.m_RAddress = value;
            }
        }

        public string SAddress
        {
            get
            {
                return this.m_SAddress;
            }
            set
            {
                this.m_SAddress = value;
            }
        }

        public string State
        {
            get
            {
                return this.m_State;
            }
            set
            {
                this.m_State = value;
            }
        }

        public int TMonths
        {
            get
            {
                return this.m_TMonths;
            }
            set
            {
                this.m_TMonths = value;
            }
        }

        public int TYears
        {
            get
            {
                return this.m_TYears;
            }
            set
            {
                this.m_TYears = value;
            }
        }

        public string Username
        {
            get
            {
                return this.m_Username;
            }
            set
            {
                this.m_Username = value;
            }
        }
    }
}

