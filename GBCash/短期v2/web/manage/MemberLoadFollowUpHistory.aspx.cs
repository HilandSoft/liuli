    ...
    public class MemberLoadFollowUpHistory : Page
    {
        ...
        private void EditButton_Click(object sender, EventArgs e)
        {
            if (this.memberID > 0)
            {
                ...
                if (byMemberID != null)
                {
                    byMemberID.NodeDesc = this.txtFollowUpDisplay.Text;
                    ebn.Edit(byMemberID);
                }
                else
                {
                    byMemberID = new MemberLoadNoteDT();
                    byMemberID.NoteContent = string.Empty;
        ...
        private void GetInfo()
        {
            if (this.memberID > 0)
            {
                ...
                if (byMemberID != null)
                {
                    this.txtFollowUpDisplay.Text = byMemberID.NodeDesc;
                }
            }
        }
