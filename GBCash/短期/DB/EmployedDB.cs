...
namespace YingNet.WeiXing.DB
{
	...
	public class EmployedDB:DBAccess
	{
		...
		public override object ConvertItem( SqlDataReader dr)
		{
			...
			if(Convert.IsDBNull(dr["IsGrossIncomeChange"])==false)
			{
				dt.IsGrossIncomeChange= Convert.ToInt32(dr["IsGrossIncomeChange"]);
			}
			if(Convert.IsDBNull(dr["IsPayOtherCredit"])==false)
			{
				dt.IsPayOtherCredit= Convert.ToInt32(dr["IsPayOtherCredit"]);
			}
			if(Convert.IsDBNull(dr["GrossIncomeChangeValue"])==false)
			{
				dt.GrossIncomeChangeValue= Convert.ToString(dr["GrossIncomeChangeValue"]);
			}
			if(Convert.IsDBNull(dr["PayOtherCreditValue"])==false)
			{
				dt.PayOtherCreditValue= Convert.ToString(dr["PayOtherCreditValue"]);
			}
		...
		public override SqlParameter[] GetParameter( object obj )
		{
			...
			SqlParameter[] prams =
			{
				...
				MakeInParam("@IsGrossIncomeChange",SqlDbType.Int,4,dt.IsGrossIncomeChange),
				MakeInParam("@IsPayOtherCredit",SqlDbType.Int,4,dt.IsPayOtherCredit),
				MakeInParam("@GrossIncomeChangeValue" ,SqlDbType.NVarChar,50,dt.GrossIncomeChangeValue),
				MakeInParam("@PayOtherCreditValue" ,SqlDbType.NVarChar,50,dt.PayOtherCreditValue)
			};
			return prams;
		}
