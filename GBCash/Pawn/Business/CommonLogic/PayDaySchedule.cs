...
namespace YingNet.WeiXing.Business.CommonLogic
{
	...
	public class PayDaySchedule
	{
		...
		/// <param name="timeAtOncePay">如果选择一次性还款的话，一次性还款的时间。如果其传入值小于2000-1-1日，那么默认为一次性还款时间为下次收入的日期</param>
			...
			ref DateTime timeFirstPay,ref DateTime timeSecondPay,ref DateTime timeThirdPay,ref DateTime timeAtOncePay,DateTime timeLoanRegistered,out string errorString)
		{
			...
			{
				if(timeAtOncePay<= new DateTime(2000,1,1))
				{
				...
				{
					if(span4.Days> numTerm)
					{
						errorString = "<script>alert(\"There's more than "+ numTerm +" days to your next payday. Please choose another repayment option.\")</script>";
						//currentPage.Response.Write("<script>alert(\"There's less than 3 days to your next payday. Please choose another repayment option.\")</script>");
						currentPage.Session["XFirst2"] = null;
						return false;
					}
			...
			else
			{
				...
				switch (numInstallmentCount)
				{
					...
					case 2:
						...
						{
							lastDueTime = Convert.ToDateTime(currentPage.Session["Datedue2"]);
						}
						break;
					case 3:
						if ((currentPage.Session["Datedue3"] != null) && !(currentPage.Session["Datedue3"].ToString() == ""))
						{
							lastDueTime = Convert.ToDateTime(currentPage.Session["Datedue3"]);
						}
						break;
				}
				TimeSpan span = (TimeSpan) (lastDueTime - SafeDateTime.LocalNow);
				int daysForInterest= span.Days+ 1;
				...
				else
				{
					...
					{
						case 1:
							num15 = (numLoanAmount + ((numLoanAmount * daysForInterest) * numInterest)) + numPoundage;
						...
						case 2:
							...
							//temp = num15.ToString("0.00");
							num16 = (((((numLoanAmount * daysForInterest) * numInterest) ) + numLoanAmount) + numPoundage) * 0.5;
						...
						case 3:
							...
							temp= (((double) (((((numLoanAmount * daysForInterest) * numInterest) ) + numLoanAmount) + numPoundage)) / 3.0).ToString("0.00");
