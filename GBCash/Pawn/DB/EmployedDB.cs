...
namespace YingNet.WeiXing.DB
{
	...
	public class EmployedDB:DBAccess
	{
		...
		public override object ConvertItem( SqlDataReader dr)
		{
			...
			if(Convert.IsDBNull(dr["ItemCondition"])==false)
			{
				dt.ItemCondition= Convert.ToString(dr["ItemCondition"]);
			}

			if(Convert.IsDBNull(dr["ItemDescription"])==false)
			{
				dt.ItemDescription= Convert.ToString(dr["ItemDescription"]);
			}

			if(Convert.IsDBNull(dr["ItemToPawn"])==false)
			{
				dt.ItemToPawn= Convert.ToString(dr["ItemToPawn"]);
			}

			if(Convert.IsDBNull(dr["ItemPhoto"])==false)
			{
				dt.ItemPhoto= Convert.ToString(dr["ItemPhoto"]);
			}

			if(Convert.IsDBNull(dr["NeedGoldPack"])==false)
			{
				dt.NeedGoldPack= Convert.ToInt32(dr["NeedGoldPack"]);
			}

			if(Convert.IsDBNull(dr["ItemPhotoNumber"])==false)
			{
				dt.ItemPhotoNumber= Convert.ToString(dr["ItemPhotoNumber"]);
			}

			if(Convert.IsDBNull(dr["OncePaymentInDays"])==false)
			{
				dt.OncePaymentInDays= Convert.ToInt32(dr["OncePaymentInDays"]);
			}
		...
		public override SqlParameter[] GetParameter( object obj )
		{
			...
			SqlParameter[] prams =
			{
				...
				MakeInParam("@ItemCondition" ,SqlDbType.NVarChar,200,dt.ItemCondition),
				MakeInParam("@ItemDescription" ,SqlDbType.NVarChar,2000,dt.ItemDescription),
				MakeInParam("@ItemToPawn" ,SqlDbType.NVarChar,200,dt.ItemToPawn),
				MakeInParam("@ItemPhoto" ,SqlDbType.NVarChar,200,dt.ItemPhoto),
				MakeInParam("@ItemPhotoNumber" ,SqlDbType.NVarChar,200,dt.ItemPhotoNumber),
				MakeInParam("@NeedGoldPack",SqlDbType.Int,4,dt.NeedGoldPack),
				MakeInParam("@OncePaymentInDays",SqlDbType.Int,4,dt.OncePaymentInDays)
			};
			return prams;
		}
