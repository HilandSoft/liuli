...
//				DateTime time = new DateTime(nextPaydayYYYY, nextPaydayMM, nextPaydayDD);
...
//				else 
					...
					if (this.Session["Repayduezs"].ToString() != this.txLoan.Value)
				{
					base.Response.Write("<script>alert(\"Please click 'Calculate' button for repayment schedule before you submit the application.\");</script>");
				}
				else
				{
					try
					{
						...
						//发送注册成功邮件
						YingNet.Common.SimpleMailSneder.SendMail(DateTime.Now.ToString()+ "Pawn For Member","please to check.",ConfigurationSettings.AppSettings["notificationMailTO"]);
...
//			dt.OtherSmallCreditCount= int.Parse(ddlSmalCreditCount.SelectedValue);
			...
			dt.ItemCondition= drpItemCondition.SelectedValue;
			dt.ItemDescription= txItemDescription.Text;
			dt.ItemToPawn= drpItems.SelectedValue;
