namespace YingNet.WeiXing.WebApp
{
    ...
    public class step1 : Page
    {
        ...
        private void bnSubmit_ServerClick(object sender, EventArgs e)
        {
            ...
            {
                this.Info = this.Info + "You must be an Australian citizen or permanent resident!&lt;br&gt;";
            }
            if (!this.CheckBoxList1.Items[1].Selected)
            {
                this.Info = this.Info + "You must be over 18 years old!&lt;br&gt;";
            }
            if (!this.CheckBoxList1.Items[2].Selected)
            {
                this.Info = this.Info + "You must have regular income!<br>";
            }
            ...
            {
                //base.Response.Write("<script>window.top.location='newcu2.htm';</script>");
				Response.Write("<script>window.top.location='newcu_instruction.htm';</script>");
            }
        }
